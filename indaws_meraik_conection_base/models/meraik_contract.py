...
class MeraikContract(models.Model):
    ...
    def action_test_connection(self):
        try:
            ...
            if not result or not isinstance(result, list):
                raise ValueError(_("Invalid response from remote server"))
            remote_data = result[0]
            self.write({
                'state': remote_data.get('state', self.state),
                'name': remote_data.get('name', self.name)
            })
    ...
    def action_view_request_responses(self):
        self.ensure_one()
        action = self.env['ir.actions.act_window']._for_xml_id('indaws_meraik_conection_base.action_meraik_request_response')
